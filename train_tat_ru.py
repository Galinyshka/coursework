...
def translate(text, trg='ru_RU', src='tt_XX', max_length=200, num_beams=5, repetition_penalty=5.0, **kwargs):
...
CURRENT_RUN = len(os.listdir(os.path.join(cur_path, 'runs_tat_ru')))
save_path = os.path.join(cur_path, 'runs_tat_ru', str(CURRENT_RUN).zfill(3))
os.mkdir(save_path)
model.cuda()
...
tokenizer.src_lang = "tt_XX"
all_data = pd.read_csv(os.path.join(cur_path, 'data/train_data.csv'))#.sample(100)
all_pairs = list(zip(all_data['Tatar'], all_data['Russian']))
test_data = pd.read_csv(os.path.join(cur_path, 'data/test_data.csv'))#.sample(10)
cleanup()
model.train()
